...
import onBoardingPage from "../../webSanityPages/onBoardingPage";
...
class OnBordingPage {
    ...
    get clickOnDownloadIcon() {
        // return $('(//android.widget.ImageView[@resource-id="com.lionsgateplay.videoapp:id/icon"])[2]');
        return $('//android.widget.ImageView[@resource-id="com.lionsgateplay.videoapp:id/icon"]')
    }
    ...
    get clickOnCloseIcon() {
        return $('//android.widget.ImageView[@content-desc="inapp_close_btn"]');
    }
    ...
    async verifyHomeScreenForSubscribedUser(username: string) {
        ...
        if (await (await this.clickOnCloseIcon).isDisplayed()) {
            await (await this.clickOnCloseIcon).combinedClick("Close Icon");
        }
        else {
            console.log("Close icon is not displayed");
        }
    ...
    */
    async verifyScrollAndHomeScreenForSubscribedUserByClickOnGetExploreBtn() {
       ...
       // await (await this.verifySignUpOrLoginTitle).findElementAndVerifyText(movieTitle);
        ...
        // const contentTitle = await (await onBoardingPage.contentDetailTitleText ).getText();
 ...
 */
    ...
    */
    async verifyHomeScreenForSubscribedUserByClickOnDownloadInShow() {
        ...
        await $('android=new UiScrollable(new UiSelector().scrollable(true)).scrollTextIntoView("3. Retention")');
        await browser.pause(3000);
        await (await this.clickOnDownloadIcon).verifyElementIsDisplayedTrue("Download Icon");
    ...
    */
    ...
    //     el.scrollTextIntoView({ behavior: 'smooth', block: 'center', inline: 'nearest' });
    //   }, await this.clickOnShowAndNavigateToDetailsScreen);
   
    //       await (await this.clickOnShowAndNavigateToDetailsScreen).scrollTextIntoView("TRAILER");
    //     await browser.pause(3000);
