...
describe(`Player Screen related Test Cases`, async () => {
  it("Validate that content playback", async () => {
    ...
    const minutelabel = skipDurationInMinutes / 10;
    let integer = Math.floor(minutelabel); // 11
    console.log(minutelabel, integer);
    const secondsPerPress = 5;
    const totalSecondsToSkip1 = skipDurationInMinutes * 60;
    const totalSecondsToSkip = totalSecondsToSkip1 + seconds;
